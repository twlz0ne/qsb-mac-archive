...
+ (id)objectWithJSONString:(NSString *)jsonString {
	...
	id value = nil;
	[scanner scanJSONValue:&value];

	// Valid files must have an object or array at the top level.
	if ([value respondsToSelector:@selector(keyEnumerator)] // Dictionary/object
			|| [value respondsToSelector:@selector(objectAtIndex:)]) { // Array
		return value;
	}
	// If it's not an object or array at the top level, reject it as invalid.
	return nil;
}
...
+ (NSString *)jsonStringForValue:(id)value withIndentLevel:(int)level
{
	...
	if ([value respondsToSelector:@selector(characterAtIndex:)]) // String
		jsonString = [BSJSON jsonStringForString:(NSString *)value];
		...
		jsonString = [BSJSON jsonStringForArray:(NSArray *)value withIndentLevel:level];
...
+ (NSString *)jsonStringForArray:(NSArray *)array withIndentLevel:(int)level
{
	...
	if ([array count] > 0) {
		[jsonString appendString:[BSJSON jsonStringForValue:[array objectAtIndex:0] withIndentLevel:level]];
	}

	unsigned int i;
	for (i = 1; i < [array count]; i++) {
		[jsonString appendFormat:@"%@ %@", jsonValueSeparatorString, [BSJSON jsonStringForValue:[array objectAtIndex:i] withIndentLevel:level]];
	}
	...
	for (i = 0; i < [string length]; i++) {
		...
		switch (nextChar) {
			...
			case '\\':
				[jsonString appendString:@"\\"];
				break;
				/* TODO: email out to json group on this - spec says to handle this, examples and example code don't handle this.
				 case '\/':
				 [jsonString appendString:@"\\/"];
				 break;
				 */
