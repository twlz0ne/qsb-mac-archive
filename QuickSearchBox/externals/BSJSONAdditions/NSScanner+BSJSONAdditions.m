...
{
  // rbjones - adding code to bypass irrelevant characters before an array
  NSString *ignoredString;
  [self scanUpToString:jsonArrayStartString intoString:&ignoredString];
      ...
      if (currentChar != '\\') {
        ...
        switch (nextChar) {
          ...
          case 'x':
            ...
            [self setScanLocation:([self scanLocation] + 2)]; // advance past '\u' or '\x'
            unsigned int charLength = 4;
            if (nextChar == 'x') charLength = 2;
            NSString *digits =
            [[self string] substringWithRange:NSMakeRange([self scanLocation], charLength)];
            /* START Updated code modified from code fix submitted by Bill Garrison - March 28, 2006 */
            /* http://www.standardorbit.net */
            /* fix as documented on http://blakeseely.com/blog/archives/2006/03/29/bsjsonadditions-12/ */
            NSScanner *hexScanner = [NSScanner scannerWithString:digits];
            NSCharacterSet *hexDigitSet =
            [NSCharacterSet characterSetWithCharactersInString:@"0123456789ABCDEFabcdef"];
            NSString *verifiedHexDigits;
            if (![hexScanner scanCharactersFromSet:hexDigitSet intoString:&verifiedHexDigits])
              return NO;
            if (charLength != [verifiedHexDigits length])
              return NO;
            
            // Read in the hex value
            [hexScanner setScanLocation:0];
            unsigned unicodeHexValue;
            if (![hexScanner scanHexInt:&unicodeHexValue])
              return NO;
            ...
            [self setScanLocation:([self scanLocation] + charLength)];
            break;
          default:
...
- (BOOL)scanJSONValue:(id *)value
{
  ...
  NSRange stringRange = NSMakeRange(scanLocation, (stringLength - scanLocation));
  NSUInteger trueLocation = [string rangeOfString:jsonTrueString 
                                          options:0 
                                            range:stringRange].location;
  NSUInteger falseLocation = [string rangeOfString:jsonFalseString 
                                           options:0 
                                             range:stringRange].location;
  NSUInteger nullLocation = [string rangeOfString:jsonNullString 
                                          options:0 
                                            range:stringRange].location;
  
  NSString *substring = [string substringWithRange:NSMakeRange(scanLocation, 1)];
...
- (BOOL)scanJSONNumber:(NSNumber **)number
{
  ...
  BOOL result = [self scanDecimal:&decimalValue];
  *number = [NSDecimalNumber decimalNumberWithDecimal:decimalValue];
  return result;
}
...
- (BOOL)scanJSONWhiteSpace
{
  ...
  while (![self isAtEnd] && [space characterIsMember:[[self string] characterAtIndex:[self scanLocation]]]) {
    [self setScanLocation:([self scanLocation] + 1)];
    result = YES;
  }
